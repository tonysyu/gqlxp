...
### Requirement: Positional Schema Arguments
**Reason**: Replacing positional arguments with explicit `--schema` flag for clarity
**Migration**: Use `gqlxp app -s <schema-id>` instead of `gqlxp <schema-file>`, or omit flag to use default/library selector
